...
class AgentExpertiseTracker:
    ...
    async def get_agent_performance_metrics(self, agent_id: str) -> Optional[AgentPerformanceMetrics]:
        ...
        # Check cache first
        if (agent_id in self.metrics_cache and 
            agent_id in self.last_cache_update and
            (datetime.now() - self.last_cache_update[agent_id]).total_seconds() < self.cache_ttl):
    ...
    async def generate_expertise_insights(self, agent_id: str) -> List[ExpertiseInsight]:
        ...
        # Check cache first
        if (agent_id in self.insights_cache and 
            agent_id in self.last_cache_update and
            (datetime.now() - self.last_cache_update[agent_id]).total_seconds() < self.cache_ttl):
    ...
    async def _analyze_learning_velocity(self, agent_id: str, conn) -> List[ExpertiseInsight]:
        ...
        try:
            ...
            for skill in fast_learners:
                insights.append(ExpertiseInsight(
                    agent_id=agent_id,
