...
class ContextEnhancementService:
    ...
    async def enhance_agent_context(
        ...
        try:
            ...
            org_knowledge = await self._gather_organizational_knowledge(
                agent_info['organization_id'], search_queries, agent_id, agent_info['team_id']
            )
            ...
            enhanced_context = EnhancedContext(
                ...
                enhancement_metadata={
                    ...
                    'total_relevance_score': sum(
                        item.relevance_score for item in org_knowledge + team_knowledge + similar_tasks
                    ),
            ...
            logger.info(f"Enhanced context for agent {agent_id}: "
    ...
    async def get_contextual_guidance(
        ...
        try:
            ...
            if current_iteration > 3:
                guidance_items.extend(await self._get_iteration_guidance(
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                similar_task_data = await conn.fetch("""
                    ...
                    LIMIT $3
                """, organization_id, task_data.get('description', ''), self.max_similar_tasks)
    ...
    async def _extract_success_patterns(
        self,
        all_knowledge: List[ContextEnhancement]
    ) -> List[str]:
