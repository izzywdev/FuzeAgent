    ...
    async def create_subscription(
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                """,
                    subscription_id, organization_id, tier.value, 
                    SubscriptionStatus.TRIAL.value if trial_end else SubscriptionStatus.ACTIVE.value,
                    billing_cycle.value, period_start, period_end, trial_end,
                    1, json.dumps(custom_limits or {}), billing_email, payment_method_id,
                    now, now, json.dumps({"created_via": "api"})
                )
    ...
    async def track_usage(
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                ...
                """,
                    usage_id, organization_id, subscription['id'],
                    metric_type.value, value, unit,
                    datetime.utcnow(), billing_period,
    ...
    async def calculate_monthly_bill(
        ...
        try:
            ...
            return {
                ...
                "billing_period": {"start": billing_period_start, "end": billing_period_end},
    ...
    async def _check_usage_limits(
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                ...
                """, organization_id, subscription_id, metric_type.value, billing_period)
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                result = await conn.execute("""
                    ...
                    WHERE organization_id = $1 
                    AND status IN ('active', 'trial')
                """, organization_id, new_tier.value, datetime.utcnow())
