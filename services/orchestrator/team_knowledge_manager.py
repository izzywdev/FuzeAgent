...
class TeamKnowledgeManager:
    ...
    async def create_team_knowledge(
        ...
        async with self.pool.acquire() as conn:
            ...
            """, 
                team_knowledge_id, team_id, org_id, title, content, content_type.value,
                knowledge_category.value, embedding, source_type.value,
                contributing_agents or [], source_knowledge_ids or [], aggregation_method,
                team_relevance_score, json.dumps(metadata or {}), tags or []
            )
    ...
    async def search_team_knowledge(
        ...
        async with self.pool.acquire() as conn:
            ...
            team_results = await self._search_team_specific_knowledge(
                conn, team_id, query_embedding, categories, content_types, limit, min_similarity
            )
            ...
            if include_org_knowledge and len(results) < limit:
                org_results = await self._search_org_knowledge_for_team(
                    conn, team_id, query, categories, content_types, 
                    limit - len(results), min_similarity
                )
    ...
    async def aggregate_agent_knowledge_to_team(
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("""
                ...
                WHERE id = ANY($1)
            """, agent_memory_ids, team_knowledge_id)
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("""
                ...
                SET effectiveness_score = $2,
                    ...
                    updated_at = NOW()
                WHERE id = $1
            """, team_knowledge_id, new_effectiveness, adoption_rate, contributing_agents)
    ...
    async def _search_team_specific_knowledge(
        ...
        for row in results:
            ...
            search_results.append(TeamKnowledgeSearchResult(
    ...
    async def _search_org_knowledge_for_team(
        ...
        for org_result in org_results:
            ...
            if team_relevance >= self.min_team_relevance:
                ...
                combined_score = (
                    org_result.similarity_score * 0.5 +
                    team_relevance * 0.5
                )
                
                team_results.append(TeamKnowledgeSearchResult(
    ...
    async def _calculate_team_relevance(
        ...
        team_usage = await conn.fetchval("""
        ...
        """, team_id, org_knowledge.id) or 0.0
