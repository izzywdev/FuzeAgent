...
class SandboxExecuteRequest(BaseModel):
    ...
    command: str = Field(
        ..., min_length=1, description="Command to execute in the sandbox"
    )
...
class AgentFromTemplateRequest(BaseModel):
    ...
    overrides: Dict[str, Any] = Field(
        default_factory=dict, description="Per-agent config overrides"
    )
...
class InteractionRequest(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Interaction metadata"
    )
...
class AgentRegistrationRequest(BaseModel):
    ...
    container_id: Optional[str] = Field(
        None, description="Container id hosting the agent"
    )
    capabilities: List[str] = Field(
        default_factory=list, description="Agent capabilities"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Registration metadata"
    )
...
class AgentErrorRequest(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(
        None, description="Optional structured details"
    )
...
class ClaudeSessionInputBody(BaseModel):
    model_config = ConfigDict(extra="forbid")
    input: str = Field(
        ..., min_length=1, description="Input to send to the Claude SDK session"
    )
...
async def update_task(
    ...
    await app.state.task_queue.update_task_status(
        task_id=task_id, status=update_data.status, result=update_data.result
    )
...
async def rollback_file_operations(
