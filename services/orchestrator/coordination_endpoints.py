    ...
    try:
        async with get_db_connection() as conn:
            ...
            """, 
                ...
                product.endpoints, product.dependencies,
                product.resource_requirements, product.team_contacts,
                product.priority_level, product.metadata,
                datetime.utcnow(), datetime.utcnow()
            )
            
        return {"status": "success", "product_id": product.id, "message": "Product registered successfully"}
...
async def create_coordination_request(request: CoordinationRequestCreate):
    ...
    try:
        ...
        if not requesting_product:
            raise HTTPException(status_code=400, detail=f"Requesting product {request.requesting_product} not found")
        ...
        async with get_db_connection() as conn:
            ...
            """, request_id, "created", "system", {"created_via": "api"})
...
async def resolve_coordination_request(
    ...
    try:
        ...
        if existing_request['status'] not in ['pending', 'in_progress']:
            raise HTTPException(status_code=400, detail=f"Request is already {existing_request['status']}")
        ...
        async with get_db_connection() as conn:
            ...
            """, 
                ...
                "agent",
                {"resolution_plan": resolution.resolution_plan, "notes": resolution.notes}
            )
...
async def allocate_resource(allocation: ResourceAllocationCreate):
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail=f"Resource {allocation.resource_name} of type {allocation.resource_type} is already allocated")
    except Exception as e:
