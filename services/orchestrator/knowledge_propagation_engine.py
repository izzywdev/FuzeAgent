...
class KnowledgePropagationEngine:
    ...
    async def trigger_team_to_org_propagation(
        self,
        team_id: str,
        knowledge_threshold_check: bool = True
    ) -> List[str]:
                ...
                try:
                    ...
                    await conn.execute("""
                        ...
                        SET propagation_status = 'failed',
                            metadata = metadata || $2
                        WHERE id = $1
                    """, task.id, json.dumps({'error': str(e)}))
                    results['failed'] += 1
    ...
    async def get_propagation_statistics(
        ...
        async with self.pool.acquire() as conn:
            ...
            # Trigger analysis
            trigger_stats = await conn.fetch(f"""
                SELECT 
                    propagation_trigger,
                    COUNT(*) as count,
                    AVG(confidence_score) as avg_confidence
        ...
        while self._running:
            try:
                ...
                if results['processed'] > 0:
                    logger.info(f"Processed {results['processed']} propagations: "
    ...
    async def _create_propagation_task(
        ...
        async with self.pool.acquire() as conn:
            ...
            """, 
                propagation_id, source_type, source_id, target_type, target_id,
                knowledge_type, propagation_trigger.value, confidence_score,
    ...
    def _meets_propagation_criteria(self, memories: List, task_outcome: Dict[str, Any]) -> bool:
        ...
        recent_memories = [
            ...
            if (datetime.now() - mem['created_at']).days <= 7
        ]
