...
class OrganizationRAGManager:
    ...
    async def initialize(self):
        ...
        try:
            self.pool = await asyncpg.create_pool(
                self.database_url, min_size=2, max_size=15, command_timeout=60
            )
        ...
        async with self.pool.acquire() as conn:
            ...
            for row in knowledge_results:
                ...
                if full_knowledge:
                    ...
                    combined_score = (
                        row["similarity_score"] * 0.7 + row["relevance_score"] * 0.3
                    )
    ...
    async def get_knowledge_by_id(
        ...
        async with self.pool.acquire() as conn:
            ...
            if not self._check_access_permissions(
                knowledge, requester_agent_id, requester_team_id
            ):
                return None
    ...
    async def cleanup_expired_knowledge(self, organization_id: str) -> int:
        ...
        async with self.pool.acquire() as conn:
            ...
            if deleted_count > 0:
                logger.info(
                    f"Cleaned up {deleted_count} expired knowledge items for org {organization_id}"
                )
    ...
    def _check_access_permissions(
        ...
        if knowledge.visibility_level == VisibilityLevel.ORGANIZATION:
            ...
            return (
                requester_team_id in knowledge.access_teams
                if requester_team_id
                else False
            )
        elif knowledge.visibility_level == VisibilityLevel.AGENT:
            return (
                requester_agent_id in knowledge.access_agents
    ...
    async def _verify_schema(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            if tables == 0:
                raise Exception(
                    "organization_knowledge_base table not found. Run migrations first."
                )
