...
async def get_hierarchy_visualization():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate hierarchy visualization: {str(e)}")
...
async def search_hierarchy(q: str, entity_type: Optional[str] = None):
    ...
    try:
        ...
        if not entity_type or entity_type == "organization":
            organizations = await DatabaseManager.get_organizations()
            for org in organizations:
                if (query in org['name'].lower() or 
                    query in org.get('description', '').lower()):
            ...
            for org in organizations:
                ...
                for team in teams:
                    if (query in team['name'].lower() or
                        query in team.get('description', '').lower() or
                        query in team.get('team_type', '').lower()):
