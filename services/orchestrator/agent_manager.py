...
class AgentManager:
    ...
    async def create_agent(
        ...
        if team_id:
            ...
            await self._register_agent_in_db(
                name,
                role,
                type,
                {
    ...
    async def _spawn_agent_container(
        self, name: str, role: str, agent_type: str, config: dict
    ) -> Optional[str]:
        ...
        image = image_map.get(
            agent_type, "ghcr.io/izzywdev/fuzeagent/claude-runner-python-dev:latest"
        )
    ...
    async def _spawn_k8s_job(
        self, agent_id: str, image: str, config: dict
    ) -> Optional[str]:
                        ...
                        containers=[
                            k8s_client.V1Container(
                                ...
                                env=[
                                    ...
                                    k8s_client.V1EnvVar(
                                        name="WS_RELAY_URL", value=ws_relay_url
                                    ),
                                    k8s_client.V1EnvVar(
        ...
        try:
            await loop.run_in_executor(
                None,
                lambda: self.k8s_batch_v1.create_namespaced_job(
    ...
    async def _spawn_docker_container_legacy(
        self, agent_id: str, image: str, config: dict
    ) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"Spawned Docker container for agent {agent_id}: {container.id}"
            )
            return container.id
        except Exception as e:
    ...
    async def _terminate_agent_container(self, agent_id: str):
        ...
        if self.k8s_batch_v1:
            job_name = (
                f"agent-{agent_id[:50].lower().replace('_', '-').replace(' ', '-')}"
            )
            loop = asyncio.get_event_loop()
            try:
                await loop.run_in_executor(
    ...
    async def _register_agent_in_db(
        self, name: str, role: str, type: str, config: dict
    ):
        ...
        async with get_db_connection() as conn:
            await conn.execute(
                ...
                type,
                {
                    ...
                    if k not in ["repository_settings", "sandbox_settings"]
                },
    ...
    async def deploy_memory_enabled_agent(
        ...
        try:
            ...
            await self._update_agent_memory_status(
                agent_id,
                "memory_enabled",
                {
                    "sandbox_id": sandbox.sandbox_id,
    ...
    async def get_agent_memory_status(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_metrics = (
                await self.expertise_tracker.get_agent_performance_metrics(agent_id)
            )

            # Get recent insights
            insights = await self.expertise_tracker.generate_expertise_insights(
                agent_id
            )
    ...
    async def get_system_expertise_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            expertise_summary = (
                await self.expertise_tracker.get_system_wide_expertise_summary()
            )
    ...
    async def _update_agent_memory_status(
        self, agent_id: str, status: str, additional_data: Dict[str, Any]
    ):
