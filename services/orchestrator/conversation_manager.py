...
class ConversationManager:
    ...
    async def store_human_interaction(
        ...
        async with get_db_connection() as conn:
            interaction_id = await conn.fetchval("""
                ...
                RETURNING id
            """,
                task_id, iteration_number, interaction_type.value,
                agent_message, human_response, 
    ...
    async def store_performance_metric(
        ...
        async with get_db_connection() as conn:
            ...
            """,
                agent_id, task_id, metric_type, metric_value,
                metric_unit, json.dumps(context) if context else {}
            )
    ...
    async def _store_conversation_session(self, session: ConversationSession) -> str:
        ...
        async with get_db_connection() as conn:
            session_id = await conn.fetchval("""
                ...
                RETURNING id
            """,
                session.agent_id, session.task_id, session.sandbox_id,
                session.session_started_at, session.total_messages,
                session.total_tokens, session.status,
    ...
    async def _update_conversation_session(self, session_id: str, session: ConversationSession):
        ...
        async with get_db_connection() as conn:
            await conn.execute("""
                ...
                WHERE id = $6
            """,
                session.session_ended_at, session.total_messages,
                session.total_tokens, session.status,
                json.dumps(session.metadata) if session.metadata else {},
                session_id
            )
    ...
    async def _store_claude_conversation(self, message: ConversationMessage) -> str:
        ...
        async with get_db_connection() as conn:
            message_id = await conn.fetchval("""
                ...
                RETURNING id
            """,
                message.task_id, message.iteration_number, 
                message.message_type.value, message.content,
                message.token_count, message.model_used, 
                message.temperature, message.response_time_ms,
