...
class RAGManager:
    ...
    async def _create_conversation_summary(self, session_id: str, agent_id: str):
        ...
        async with self.pool.acquire() as conn:
            ...
            try:
                ...
                summary_response = await self.llm.ainvoke([HumanMessage(content=formatted_prompt)])
                ...
                time_range = f"[{messages[0]['created_at']}, {messages[-1]['created_at']}]"
        ...
        async with self.pool.acquire() as conn:
            ...
            if include_summaries:
                summaries = await conn.fetch(f"""
                    ...
                    ORDER BY similarity DESC
                    LIMIT ${param_offset + 1}
                """, *params, similarity_threshold, limit // 2)
    ...
    async def get_conversation_context(
        ...
        async with self.pool.acquire() as conn:
            ...
            recent_messages = await conn.fetch("""
                ...
                ORDER BY created_at DESC
                LIMIT $3
            """, session_id, agent_id, context_window)
    ...
    async def cleanup_old_data(self, retention_days: int = 90):
        ...
        async with self.pool.acquire() as conn:
            ...
            logger.info(f"Archived {len(archived_sessions)} sessions and deleted {deleted_conversations} old conversations")
