...
class FileOperationsEngine:
    ...
    async def apply_operations_if_approved(self, batch_id: str) -> bool:
        ...
        if batch.approval_status in [ApprovalStatus.APPROVED, ApprovalStatus.AUTO_APPROVED]:
            return await self._apply_operations(batch)
    ...
    async def _rollback_operation(self, operation: FileOperation, rollback_info: Dict[str, Any]):
        ...
        elif operation.operation in [OperationType.MODIFY_FILE, OperationType.DELETE_FILE]:
            # Restore from backup
