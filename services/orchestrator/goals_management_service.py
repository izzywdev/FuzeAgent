...
class GoalsManagementService:
    ...
    async def create_goal(
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                """, 
                    goal_id, organization_id, title, description, goal_type.value,
                    priority_level, target_value, target_unit, json.dumps(success_criteria),
                    target_deadline, GoalStatus.ACTIVE.value, assigned_teams, goal_owner_agent_id,
                    stakeholder_agents, tags, json.dumps(metadata), created_by
                )
                ...
                if target_value is not None and target_unit is not None:
                    ...
                    """, 
                        goal_id, f"Primary {target_unit} Target", 
                        self._determine_metric_type(target_unit),
                        target_unit, Decimal('0'), target_value, Decimal('0')
                    )
                ...
                """, 
                    goal_id, 'goal_created', Decimal('0'), 
                    f'Goal created: {title}', created_by, 'system'
                )
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if update_fields:
                    ...
                    """, 
                        goal_id, 'manual_update', progress_percentage, current_value,
                        progress_notes or 'Progress updated manually', recorded_by, 'manual'
                    )
    ...
    async def create_milestone(
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                """, 
                    milestone_id, goal_id, parent_milestone_id, title, description, milestone_type,
                    target_date, json.dumps(success_criteria), json.dumps(deliverables), 
                    json.dumps(dependencies), assigned_teams, responsible_agent_id, 
                    priority_level, weight_in_goal, created_by
                )
    ...
    async def create_task_from_milestone(
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                """, 
                    task_id, str(goal_id), milestone_id, title, description, task_type,
                    complexity_level, estimated_hours, due_date, assigned_team_id,
                    assigned_agent_id, priority, json.dumps(dependencies), 
                    json.dumps(requirements), json.dumps(acceptance_criteria), created_by_agent_id
                )
            
            self.tasks_created += 1
            logger.info(f"Created task {task_id}: {title} from milestone {milestone_id}")
            ...
            async with self.pool.acquire() as conn:
                ...
                return {
                    ...
                    'summary': {
                        ...
                        'overdue_tasks': len([t for t in tasks if t['due_date'] and t['due_date'] < date.today() and t['status'] not in ['completed', 'cancelled']]),
                    }
                }
    ...
    async def get_organization_goals_dashboard(self, organization_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting organization dashboard for {organization_id}: {e}")
            return {'error': str(e)}
    ...
    def _row_to_goal(self, row) -> OrganizationGoal:
        ...
        return OrganizationGoal(
            ...
            success_criteria=json.loads(row['success_criteria']) if isinstance(row['success_criteria'], str) else row['success_criteria'],
            ...
            goal_owner_agent_id=str(row['goal_owner_agent_id']) if row['goal_owner_agent_id'] else None,
