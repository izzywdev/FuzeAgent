...
class AgentSandboxManager:
        ...
        async with get_db_connection() as conn:
            if destroyed_at:
                await conn.execute("""
                    ...
                    WHERE id = $1
                """, sandbox_id, status.value, container_id, destroyed_at)
            else:
                await conn.execute("""
                    UPDATE agent_sandboxes 
                    SET status = $2, container_id = $3
                    WHERE id = $1
                """, sandbox_id, status.value, container_id)
    ...
    async def _cleanup_expired_sandboxes(self):
        ...
        expired_sandboxes = [
            sandbox for sandbox in self.active_sandboxes.values()
