...
class KnowledgeNotificationService:
    ...
    async def notify_new_knowledge(
        ...
        try:
            ...
            recipients = await self._identify_knowledge_recipients(
                knowledge_type, organization_id, source_team_id, source_agent_id, knowledge_category
            )
    ...
    async def notify_knowledge_conflict(
        ...
        try:
            ...
            if affected_teams:
                recipients = [(otype, oid) for otype, oid in [('team', team_id) for team_id in affected_teams]]
            else:
            ...
            for recipient_type, recipient_id in recipients:
                ...
                message = f"Conflicting knowledge items detected: {conflict_description}"
                ...
                notification_id = await self._create_notification(
                    ...
                    title=title,
                    message=message,
                    knowledge_id=conflicting_knowledge_ids[0] if conflicting_knowledge_ids else None,
                    knowledge_type="conflict",
                    priority=Priority.HIGH,
                    requires_action=True,
                    suggested_actions=[
                        {"action": "review_conflict", "label": "Review Conflicting Knowledge"},
    ...
    async def notify_relevant_knowledge_for_task(
        ...
        try:
            ...
            message = f"Found {len(relevant_knowledge)} knowledge items that might help with your current task:\n\n" + "\n".join(knowledge_summary)
            ...
            notification_id = await self._create_notification(
                ...
                expires_at=datetime.now() + timedelta(hours=24)  # Task-specific, expires quickly
            )
        ...
        try:
            for target_team_id in target_team_ids:
                ...
                notification_id = await self._create_notification(
                    ...
                    suggested_actions=[
                        {"action": "review_knowledge", "label": "Review Knowledge"},
                        {"action": "request_sharing", "label": "Request Knowledge Sharing"},
                        {"action": "schedule_meeting", "label": "Schedule Knowledge Transfer"}
                    ],
                    metadata={
    ...
    async def get_notifications_for_recipient(
        ...
        try:
            ...
            logger.error(f"Error getting notifications for {recipient_type} {recipient_id}: {e}")
            return []
    ...
    async def get_notification_statistics(
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                priority_stats = await conn.fetch(f"""
                    SELECT 
                        priority,
                        COUNT(*) as count,
    ...
    async def _create_notification(
        ...
        if not self._check_rate_limits(recipient_type, recipient_id, notification_type):
            logger.debug(f"Rate limit exceeded for {recipient_type} {recipient_id}, skipping notification")
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                """, 
                    notification_id, recipient_type, recipient_id, notification_type.value,
                    title, message, knowledge_id, knowledge_type, priority.value,
                    requires_action, json.dumps(suggested_actions or []), 
            ...
            logger.info(f"Created {notification_type.value} notification for {recipient_type} {recipient_id}")
            ...
            async with self.pool.acquire() as conn:
                ...
                if knowledge_category:
                    ...
                    relevant_teams = await conn.fetch("""
                        ...
                        LIMIT 10
                    """, organization_id, knowledge_category)
                ...
                recipients = [
                    (rtype, rid) for rtype, rid in recipients 
                    if not (rtype == 'team' and rid == source_team_id) and 
                       not (rtype == 'agent' and rid == source_agent_id)
                ]
    ...
    async def _create_new_knowledge_message(
        ...
        message_parts.append(f"New {knowledge_type} knowledge has been added to the organization knowledge base.")
        ...
        message_parts.append("This knowledge may be relevant to your current and future work.")
    ...
    def _create_default_notification_rules(self) -> List[NotificationRule]:
        ...
        return [
            ...
            NotificationRule(
                recipient_type='agent',
                notification_types=[NotificationType.RELEVANT_KNOWLEDGE, NotificationType.KNOWLEDGE_REQUEST],
        ...
        while self._running:
            try:
                ...
                if self.batch_notifications:
                    for recipient_key, notifications in self.batch_notifications.items():
                        if len(notifications) >= self.batch_size:
                            # Process batch
                            await self._process_notification_batch(recipient_key, notifications)
            ...
            try:
                async with self.pool.acquire() as conn:
                    ...
                    if deleted_expired > 0 or deleted_old > 0:
                        logger.info(f"Cleaned up {deleted_expired} expired and {deleted_old} old notifications")
    ...
    async def _process_notification_batch(self, recipient_key: str, notifications: List[KnowledgeNotification]):
        """Process a batch of notifications"""
        # This would implement batching logic, e.g., email digests
        self.notifications_batched += len(notifications)
        logger.info(f"Processed batch of {len(notifications)} notifications for {recipient_key}")
