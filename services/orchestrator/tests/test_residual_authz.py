...
def build_bola_app() -> FastAPI:
    ...
    async def approve(task_id: str, batch_id: str, user: CurrentUser = Depends(require_user)):
    ...
    async def rollback(task_id: str, batch_id: str, user: CurrentUser = Depends(require_user)):
        await _authorize_task_org(task_id, user)
    ...
    async def send_message(sender_agent_id: str, message_data: dict = Body(...), user: CurrentUser = Depends(require_user)):
    ...
    async def task_status(task_id: str, status_data: dict = Body(...), user: CurrentUser = Depends(require_user)):
        await _authorize_task_org(task_id, user)
...
def test_approve_non_owner_is_403(bola_client):
    r = bola_client.post("/tasks/t1/file-operations/b1/approve", headers=auth_header(organizations=[ORG_B]))
    assert r.status_code == 403


def test_approve_owner_is_200(bola_client):
    r = bola_client.post("/tasks/t1/file-operations/b1/approve", headers=auth_header(organizations=[ORG_A]))
    assert r.status_code == 200


def test_rollback_non_owner_is_403(bola_client):
    r = bola_client.post("/tasks/t1/file-operations/b1/rollback", headers=auth_header(organizations=[ORG_B]))
    assert r.status_code == 403


def test_rollback_owner_is_200(bola_client):
    r = bola_client.post("/tasks/t1/file-operations/b1/rollback", headers=auth_header(organizations=[ORG_A]))
    assert r.status_code == 200
...
def test_ws_org_non_member_is_rejected(ws_client):
    ...
    with pytest.raises(WebSocketDisconnect):
        with ws_client.websocket_connect(f"/ws/organization/{ORG_A}/updates?token={token}"):
            pass


def test_ws_org_member_connects(ws_client):
    token = make_token(organizations=[ORG_A])
    with ws_client.websocket_connect(f"/ws/organization/{ORG_A}/updates?token={token}") as ws:
        data = ws.receive_json()
