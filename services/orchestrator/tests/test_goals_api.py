...
from services.orchestrator.goals_management_service import GoalType, GoalStatus, OrganizationGoal
from services.orchestrator.goal_conversation_service import ConversationType, ConversationStatus
...
class TestPlanningEngineAPI:
    ...
    def test_generate_weekly_tasks(self, mock_engine):
        ...
        response = client.post(
            "/milestones/milestone-123/generate-weekly-tasks",
            json=focus_areas
        )
    ...
    def test_generate_cross_functional_tasks(self, mock_engine):
        ...
        response = client.post(
            "/goals/goal-123/generate-cross-functional-tasks",
...
class TestGoalsAPIIntegration:
    ...
    @patch('services.orchestrator.main.app.state.milestone_task_engine')
    @patch('services.orchestrator.main.app.state.goal_conversation_service')
    def test_complete_goal_workflow(self, mock_conv_service, mock_engine, mock_goals_service, 
                                   sample_goal_data, sample_conversation_data):
        ...
        mock_engine.generate_goal_execution_plan = AsyncMock(return_value={
            "goal_id": "goal-123",
            "milestones": [],
            "summary": {}
        })
