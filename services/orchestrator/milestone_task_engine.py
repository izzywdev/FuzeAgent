...
class MilestoneTaskEngine:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                while current_date <= end_date:
                    ...
                    """, 
                        str(uuid.uuid4()), goal_id, milestone_title, milestone_description,
                        'metric', milestone_date,
                        json.dumps({
    ...
    async def generate_cross_functional_tasks(
        ...
        if target_functions is None:
            target_functions = ['development', 'marketing', 'sales', 'operations', 'finance']
        ...
        return {
            'business': {
                ...
                'growth': {
                    ...
                    'priority_level': 8,
                    'success_criteria': {'kpi_targets_met': True, 'growth_metrics_positive': True},
                    ...
                    'task_templates': [
                        {
                            ...
                            'requirements': {'budget_approved': True, 'campaign_strategy': 'defined'},
                ...
                'delivery': {
                    ...
                    'priority_level': 7,
                    'success_criteria': {'optimization_targets_met': True, 'scalability_proven': True},
                    'deliverables': [{'type': 'optimization', 'description': 'Operations optimized and scaled'}],
                    'task_templates': [
                        {
            ...
            'technical': {
                'foundation': {
                    ...
                    'weight_in_goal': Decimal('20.0'),
                    'priority_level': 9,
                    'success_criteria': {'architecture_defined': True, 'dev_environment_ready': True},
                    'deliverables': [{'type': 'technical_spec', 'description': 'Technical architecture documented'}],
                    'task_templates': [
                        {
    ...
    async def _create_milestone_from_plan(self, goal_id: str, milestone_data: Dict[str, Any]) -> str:
        ...
        async with self.pool.acquire() as conn:
            ...
            """, 
                milestone_id, goal_id, milestone_data['title'], milestone_data['description'],
                milestone_data['milestone_type'], milestone_data['target_date'],
                json.dumps(milestone_data['success_criteria']), milestone_data['weight_in_goal'],
                milestone_data['priority_level'], json.dumps({'generated_from_plan': True})
            )
    ...
    async def _create_task_from_plan(self, goal_id: str, milestone_id: str, task_data: Dict[str, Any]) -> str:
        ...
        async with self.pool.acquire() as conn:
            ...
            """, 
                task_id, goal_id, milestone_id, task_data['title'], task_data['description'],
                task_data['task_type'], task_data['complexity_level'], task_data['estimated_hours'],
                task_data['due_date'], task_data['priority'], json.dumps(task_data['requirements']),
                json.dumps(task_data['acceptance_criteria']), json.dumps({'generated_from_plan': True})
            )
