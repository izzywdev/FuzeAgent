...
class GoalTrackingService:
    ...
    def __init__(self, database_url: str):
        ...
        # Configuration
        self.progress_update_threshold = Decimal('1.0')  # Minimum change to trigger update
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if progress_percentage is not None:
                    velocity = await self._calculate_velocity(goal_id, progress_percentage)
                ...
                """, 
                    snapshot_id, goal_id, milestone_id, 'manual_update',
                    progress_percentage, current_value, notes, recorded_by, 'tracking_service',
                    json.dumps({
                ...
                if progress_percentage is not None and abs(
                    ...
                    await conn.execute("""
                        ...
                        SET progress_percentage = $2,
                            ...
                            updated_at = NOW()
                        WHERE id = $1
                    """, goal_id, progress_percentage, current_value, confidence_score)
            ...
            logger.info(f"Recorded progress update for goal {goal_id}: {progress_percentage}%")
    ...
    async def assess_goal_deadline_risk(self, goal_id: str) -> DeadlineRisk:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                velocity = await self._calculate_velocity(goal_id, goal_data['progress_percentage'])
                ...
                if velocity and velocity > 0:
                    ...
                    probability_of_delay = max(0, min(1, (estimated_days_needed - days_remaining) / max(days_remaining, 1)))
                else:
                    estimated_days_needed = days_remaining * 2  # Conservative estimate
                    probability_of_delay = Decimal('0.8') if days_remaining < 30 else Decimal('0.5')
                ...
                elif probability_of_delay >= Decimal('0.3') or goal_data['overdue_tasks'] > 0:
                    risk_level = RiskLevel.MEDIUM
                else:
                    risk_level = RiskLevel.LOW
                
                # Identify critical path items
                critical_path_items = await self._identify_critical_path(goal_id, goal_data)
                ...
                if velocity and velocity > 0:
                    estimated_completion_date = date.today() + timedelta(days=estimated_days_needed)
                else:
                    estimated_completion_date = goal_data['target_deadline'] + timedelta(days=30)
    ...
    async def generate_progress_report(
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                """, goal_id, report_period_days)
                
                # Calculate metrics
                velocity = await self._calculate_velocity(goal_id, goal['progress_percentage'])
                deadline_risk = await self.assess_goal_deadline_risk(goal_id)
                
                # Performance analysis
                performance_metrics = {
                    'average_daily_progress': self._calculate_average_daily_progress(progress_history),
                    'progress_consistency': self._calculate_progress_consistency(progress_history),
                    'velocity_trend': self._analyze_velocity_trend(progress_history),
                    'milestone_completion_rate': goal['completed_milestones'] / max(goal['total_milestones'], 1) * 100,
                    'task_completion_rate': goal['completed_tasks'] / max(goal['total_tasks'], 1) * 100
                }
                ...
                # Key insights
                insights = self._extract_progress_insights(goal, progress_history, performance_metrics)
    ...
    async def get_organization_tracking_dashboard(
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                # Risk distribution
                risk_distribution = {
                    'low': len([g for g in goals_with_risk if g['deadline_risk']['risk_level'] == 'low']),
                    'medium': len([g for g in goals_with_risk if g['deadline_risk']['risk_level'] == 'medium']),
                    'high': len([g for g in goals_with_risk if g['deadline_risk']['risk_level'] == 'high']),
                    'critical': len([g for g in goals_with_risk if g['deadline_risk']['risk_level'] == 'critical'])
                }
                ...
                return {
                    ...
                    'high_priority_alerts': len([g for g in goals_with_risk if g['deadline_risk']['risk_level'] in ['high', 'critical']])
                }
                
        except Exception as e:
            logger.error(f"Error generating tracking dashboard for organization {organization_id}: {e}")
            return {'error': str(e)}
    
    # Helper methods for calculations and analysis
    
    async def _calculate_velocity(self, goal_id: str, current_progress: Decimal) -> Optional[Decimal]:
            ...
            async with self.pool.acquire() as conn:
                ...
                progress_data = await conn.fetch("""
                    ...
                    WHERE goal_id = $1 
                      AND recorded_at >= NOW() - INTERVAL '%s days'
                    ORDER BY recorded_at ASC
                """, goal_id, self.velocity_calculation_window_days)
                ...
                time_diff = (last_point['recorded_at'] - first_point['recorded_at']).days
                if time_diff <= 0:
                    return None
                
                progress_diff = last_point['progress_percentage'] - first_point['progress_percentage']
    ...
    async def _calculate_performance_indicators(
        ...
        return {
            'progress_health': 'good' if progress_percentage and progress_percentage > 50 else 'needs_attention',
            'completion_likelihood': 'high' if progress_percentage and progress_percentage > 75 else 'moderate',
    ...
    async def _identify_critical_path(self, goal_id: str, goal_data: Dict[str, Any]) -> List[Dict[str, Any]]:
                ...
                async with self.pool.acquire() as conn:
                    ...
                    for goal in active_goals:
                        try:
                            ...
                            logger.error(f"Error in periodic risk assessment for goal {goal['id']}: {e}")
                
                logger.info(f"Completed periodic risk assessment for {len(active_goals)} goals")
    ...
    def _calculate_average_daily_progress(self, progress_history: List[Dict[str, Any]]) -> float:
        """Calculate average daily progress from history"""
        if len(progress_history) < 2:
            return 0.0
        
        total_days = (progress_history[-1]['recorded_at'] - progress_history[0]['recorded_at']).days
        if total_days <= 0:
            return 0.0
        
        progress_change = progress_history[-1]['progress_percentage'] - progress_history[0]['progress_percentage']
        return float(progress_change / total_days)
    
    def _calculate_progress_consistency(self, progress_history: List[Dict[str, Any]]) -> str:
        ...
        for i in range(1, len(progress_history)):
            ...
            daily_change = (curr['progress_percentage'] - prev['progress_percentage']) / days_diff
    ...
    def _analyze_velocity_trend(self, progress_history: List[Dict[str, Any]]) -> str:
        ...
        def calc_velocity(segment):
            ...
            progress = segment[-1]['progress_percentage'] - segment[0]['progress_percentage']
    ...
    def _generate_progress_recommendations(
        ...
        if deadline_risk.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
            recommendations.append("URGENT: Goal is at high risk of missing deadline. Consider immediate intervention.")
        
        if performance_metrics['progress_consistency'] == 'inconsistent':
            recommendations.append("Improve progress consistency by establishing regular check-ins and milestone reviews.")
        
        if performance_metrics['velocity_trend'] == 'decelerating':
            recommendations.append("Progress velocity is declining. Investigate blockers and resource constraints.")
        
        if goal['completed_tasks'] / max(goal['total_tasks'], 1) < 0.5:
            recommendations.append("Focus on task completion rate. Many tasks remain incomplete.")
    ...
    def _extract_progress_insights(
        ...
        if len(progress_history) > 0:
            ...
            insights.append(f"Latest progress update: {latest_progress['progress_percentage']}% on {latest_progress['recorded_at'].strftime('%Y-%m-%d')}")
        
        if performance_metrics['average_daily_progress'] > 0:
            days_to_completion = (100 - float(goal['progress_percentage'])) / performance_metrics['average_daily_progress']
            insights.append(f"At current pace, goal completion estimated in {int(days_to_completion)} days")
        ...
        if goal['days_remaining'] is not None:
            if goal['days_remaining'] < 0:
                ...
                insights.append(f"Goal deadline approaching in {goal['days_remaining']} days")
