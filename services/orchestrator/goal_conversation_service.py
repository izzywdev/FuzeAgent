...
class GoalConversationService:
            ...
            async with self.pool.acquire() as conn:
                ...
                enhanced_context = {
                    **initial_context,
                    ...
                    "days_to_deadline": (
                        goal["target_deadline"] - datetime.now().date()
                    ).days,
    ...
    async def get_conversation(self, conversation_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                # Get insights
                insights = (
                    json.loads(conversation["insights_generated"])
                    if conversation["insights_generated"]
                    else []
                )

                # Get action items
                action_items = (
                    json.loads(conversation["action_items"])
    ...
    async def generate_planning_milestones(
        self, conversation_id: str, planning_context: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                milestone_insight = {
                    ...
                    "supporting_messages": [
                        msg["id"] for msg in messages[-5:] if "id" in msg
                    ],  # Last 5 messages
                    "suggested_actions": [
                        {
                ...
                await self._add_insight_to_conversation(
                    conversation_id, milestone_insight
                )
    ...
    async def conduct_progress_review(
        self, conversation_id: str, review_period_days: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                review_analysis = {
                    ...
                    "milestone_summary": {
                        row["status"]: row["count"] for row in milestone_status
                    },
                    "task_summary": {
                        row["status"]: row["count"] for row in task_status
                    },
                    "progress_trend": self._calculate_progress_trend(
                        [dict(p) for p in progress_data]
                    ),
    ...
    async def extract_action_items_from_conversation(
        self, conversation_id: str, auto_assign: bool = True
    ) -> List[Dict[str, Any]]:
...
Include who should be responsible and when it should be done.""",
            ...
            return [
                {
                    ...
                    "target_date": (datetime.now() + timedelta(days=timeline_days // 4))
                    .date()
                    .isoformat(),
                ...
                },
                {
                    ...
                    "target_date": (datetime.now() + timedelta(days=timeline_days // 2))
                    .date()
                    .isoformat(),
                ...
                },
                {
                    ...
                    "target_date": (
                        datetime.now() + timedelta(days=timeline_days * 3 // 4)
                    )
                    .date()
                    .isoformat(),
    ...
    def _assess_goal_risks(
        self, goal: Dict[str, Any], progress_data: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_progress_recommendations(
        ...
        if progress < 25:
            recommendations.append(
                "Consider breaking down remaining work into smaller, more manageable tasks"
            )

        if progress > 75:
            recommendations.append(
                "Focus on final quality checks and prepare for goal completion"
            )

        return recommendations

    def _identify_action_items_in_messages(
