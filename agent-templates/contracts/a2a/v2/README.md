# Fuze A2A contract v2 — DRAFT

**Contract version 2.0.0** · frozen against **A2A specification 1.0.0** (`lf.a2a.v1`,
canonical source `specification/a2a.proto` in [`a2aproject/A2A`](https://github.com/a2aproject/A2A)).

> **v1 remains frozen and supported at `../v1` (1.2.0). It is not deprecated by this document.**
> v2 exists because caller identity changed shape — see `authz.md` §2. Everything else is carried
> over unchanged, so a v1 reader will find v2 familiar; the difference is that the callee now
> receives a `(actor, principal)` pair and authorises on both.
...
card   = A2AClient.fetch_card("http://a2a-fuzeagent.fuzeagent.svc.cluster.local:8080")
