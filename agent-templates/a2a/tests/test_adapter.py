...
class FakeProvider:
    ...
    def create_session(self, agent_id, version, environment_id, vault_ids=None,
                       memory_resources=None, title=None):
        self._n += 1
        sid = f"sess-{self._n}"
...
def test_send_message_completed(fuzeplan_cfg, resolver):
    ...
    params = {"tenant": "FuzePlan", "message": {"messageId": "m1", "role": "ROLE_USER",
              "parts": [{"text": "Create tickets"}], "metadata": {"skillId": "product-manager"}}}
...
def test_unauthorized_caller_rejected(fuzeplan_cfg, resolver):
    prov = FakeProvider()
    a = _adapter(fuzeplan_cfg, prov, resolver)
    params = {"tenant": "FuzePlan", "message": {"messageId": "m", "role": "ROLE_USER",
              "parts": [{"text": "x"}]}}
...
def test_unknown_tenant_is_generic_rejected(fuzeplan_cfg, resolver):
    prov = FakeProvider()
    a = _adapter(fuzeplan_cfg, prov, resolver)
    params = {"tenant": "Nope", "message": {"messageId": "m", "role": "ROLE_USER",
              "parts": [{"text": "x"}]}}
    out = a.send_message(params, _ctx())
    assert out["task"]["status"]["state"] == "TASK_STATE_REJECTED"
...
def test_unknown_skill_rejected(fuzeplan_cfg, resolver):
    ...
    a = _adapter(fuzeplan_cfg, prov, resolver)
    params = {"tenant": "FuzePlan", "message": {"messageId": "m", "role": "ROLE_USER",
              "parts": [{"text": "x"}], "metadata": {"skillId": "does-not-exist"}}}
    out = a.send_message(params, _ctx())
    assert out["task"]["status"]["state"] == "TASK_STATE_REJECTED"
...
def test_input_required_then_continue_confirms_tool(fuzeplan_cfg, resolver):
    prov = FakeProvider(results={
        "sess-1": [
            {"text": "May I create 12 tickets?", "status": "blocked",
             "pending": {"event_ids": ["tu-1"], "tools": {"tu-1": "create_tickets({\"n\":12})"}}},
            {"text": "created", "status": "idle", "pending": None},
        ]
    })
    a = _adapter(fuzeplan_cfg, prov, resolver)
    p1 = {"tenant": "FuzePlan", "message": {"messageId": "m1", "role": "ROLE_USER",
          "parts": [{"text": "Create 12 tickets"}], "metadata": {"skillId": "product-manager"}}}
    t1 = a.send_message(p1, _ctx())["task"]
    assert t1["status"]["state"] == "TASK_STATE_INPUT_REQUIRED"
    sid = t1["id"]

    # caller answers on the SAME taskId
    p2 = {"tenant": "FuzePlan", "message": {"messageId": "m2", "role": "ROLE_USER",
          "taskId": sid, "parts": [{"text": "yes, go ahead"}]}}
...
def test_continue_deny_passes_reason(fuzeplan_cfg, resolver):
    ...
    a = _adapter(fuzeplan_cfg, prov, resolver)
    p1 = {"tenant": "FuzePlan", "message": {"messageId": "m1", "role": "ROLE_USER",
          "parts": [{"text": "open a PR"}]}}
    sid = a.send_message(p1, _ctx())["task"]["id"]
    p2 = {"tenant": "FuzePlan", "message": {"messageId": "m2", "role": "ROLE_USER",
          "taskId": sid, "parts": [{"text": "deny - too risky"}]}}
...
def test_continue_foreign_task_is_not_found(fuzeplan_cfg, resolver):
    prov = FakeProvider(results={"sess-1": [
        {"text": "?", "status": "blocked", "pending": {"event_ids": ["t"], "tools": {"t": "x"}}}]})
    a = _adapter(fuzeplan_cfg, prov, resolver)
    sid = a.send_message({"tenant": "FuzePlan", "message": {"messageId": "m", "role": "ROLE_USER",
          "parts": [{"text": "x"}]}}, _ctx(caller="FuzeSales"))["task"]["id"]
    # a DIFFERENT allowlisted caller may not touch it
    from a2a.wire_errors import TaskNotFoundError
    with pytest.raises(TaskNotFoundError):
        a.send_message({"tenant": "FuzePlan", "message": {"messageId": "m2", "role": "ROLE_USER",
              "taskId": sid, "parts": [{"text": "yes"}]}}, _ctx(caller="FuzeService"))
...
def test_list_tasks_only_own(fuzeplan_cfg, resolver):
    prov = FakeProvider()
    a = _adapter(fuzeplan_cfg, prov, resolver)
    a.send_message({"tenant": "FuzePlan", "message": {"messageId": "m", "role": "ROLE_USER",
          "parts": [{"text": "x"}]}}, _ctx(caller="FuzeSales"))
    a.send_message({"tenant": "FuzePlan", "message": {"messageId": "m", "role": "ROLE_USER",
          "parts": [{"text": "y"}]}}, _ctx(caller="FuzeService"))
